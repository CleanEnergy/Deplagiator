...
namespace AlgorithmCollection
{
    ...
    {
        /// <summary>
        /// Takes non-formatted raw texts and compares them word-by-word. When percentages are calculated, the <para>leftContent</para> is taken as the base.
        /// </summary>
        ...
        public static StringMatchingResult Start(string leftContent, string rightContent)
        {
            ...
            // Split on each word
            string[] whitespaceSplitLeft = leftContent.Split(' ');
            string[] whitespaceSplitRight = rightContent.Split(' ');
            ...
            for (int i = 0; i < whitespaceSplitRight.Length; i++)
            {
                ...
                {
                    //if (!result.MatchingWords.Contains(whitespaceSplitRight[i].ToLower()))
                    {
            ...
            result.PercentMatch = ((float)result.MatchingWords.Count * 100) / (float)hashTable.Count;
